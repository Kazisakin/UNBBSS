...
export const verifyWithdrawalOtp = async (req: Request, res: Response) => {
  try {
...
export const getNominationDetails = async (req: Request, res: Response) => {
  try {
...
export const submitWithdrawal = async (req: Request, res: Response) => {
  try {
    ...
    if (nomination.isWithdrawn || (nomination.withdrawnPositions && (nomination.withdrawnPositions as string[]).length > 0)) {
    ...
    const originalPositions = nomination.positions as string[];
    ...
    const emailContent = `
      <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
        ...
        <p>Your ${
          withdrawalType === 'complete' ? 'complete' : 'partial'
        } withdrawal from <strong>${nomination.event.name}</strong> has been processed successfully.</p>
