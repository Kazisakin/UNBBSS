...
exports.submitVote = exports.verifyVotingOtp = exports.requestVotingOtp = exports.getVotingEventDetails = void 0;
...
// Validation schemas
const requestVoteOtpSchema = zod_1.z.object({
    email: zod_1.z.string().email().refine(email => email.endsWith('@unb.ca')),
    slug: zod_1.z.string().min(1),
});
const verifyVoteOtpSchema = zod_1.z.object({
    token: zod_1.z.string().min(1),
    otp: zod_1.z.string().length(6).regex(/^\d{6}$/),
});
const submitVoteSchema = zod_1.z.object({
    voterFirstName: zod_1.z.string().min(1).max(50).regex(/^[a-zA-Z\s'-]+$/),
    voterLastName: zod_1.z.string().min(1).max(50).regex(/^[a-zA-Z\s'-]+$/),
    voterStudentId: zod_1.z.string().regex(/^\d{7}$/).refine(id => parseInt(id) > 3000000),
    voterFaculty: zod_1.z.string().min(1).max(100).regex(/^[a-zA-Z\s'-]+$/),
    voterYear: zod_1.z.enum(['1st Year', '2nd Year', '3rd Year', '4th Year', '5th Year']),
    ballot: zod_1.z.record(zod_1.z.string(), zod_1.z.string()), // {position: candidateId}
});
const generateOTP = () => {
...
const getVotingEventDetails = async (req, res) => {
    try {
        ...
        positions.forEach(position => {
            candidatesByPosition[position] = event.candidates.filter(c => c.positions.includes(position));
        });
        res.json({
            event: {
                id: event.id,
...
exports.getVotingEventDetails = getVotingEventDetails;
const requestVotingOtp = async (req, res) => {
    try {
        const { email, slug } = requestVoteOtpSchema.parse(req.body);
        const clientIP = (0, locationService_1.getClientIP)(req);
        const event = await prisma.votingEvent.findFirst({
            where: { slug, isActive: true }
        });
        if (!event) {
      ...
      `,
        ...
        (0, locationService_1.logSecurityEvent)('VOTING_OTP_REQUESTED', clientIP, (0, locationService_1.getUserAgent)(req), {
            email: email.split('@')[0] + '@***',
            eventId: event.id,
        });
        res.json({ token });
...
exports.requestVotingOtp = requestVotingOtp;
const verifyVotingOtp = async (req, res) => {
    try {
        ...
        (0, locationService_1.logSecurityEvent)('VOTING_OTP_VERIFIED', req.ip || 'unknown', (0, locationService_1.getUserAgent)(req), {
...
exports.verifyVotingOtp = verifyVotingOtp;
const submitVote = async (req, res) => {
    try {
        ...
        const clientIP = (0, locationService_1.getClientIP)(req);
        const userAgent = (0, locationService_1.getUserAgent)(req);
        const location = await (0, locationService_1.getLocationFromIP)(clientIP);
        ...
        const ballotDetails = await Promise.all(Object.entries(voteData.ballot).map(async ([position, candidateId]) => {
        ...
        <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; background: #1a1a1a; color: #ffffff; padding: 20px; border-radius: 10px;">
          ...
          <div style="background: #2d2d2d; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            ${ballotDetails.map(({ position, candidate }) => `<p style="color: #d1d5db; margin: 5px 0;"><strong>${position}:</strong> ${candidate}</p>`).join('')}
          </div>
      ...
      `,
        ...
        (0, locationService_1.logSecurityEvent)('VOTE_SUBMITTED', clientIP, userAgent, {
